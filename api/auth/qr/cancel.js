const { cancelQrLoginRequest } = require("../../_lib/qr-login");
...
module.exports = async function handler(req, res) {
  ...
  const endpoint = "/api/auth/qr/cancel";
  try {
    methodAllowed(req, ["POST"]);
    const payload = await readJsonBody(req, { maxBytes: 2048 });
    await assertRateLimit("cancelQrLoginRequest", getClientIp(req));
    const data = await cancelQrLoginRequest({ payload, context: { requestId: id, endpoint } });
