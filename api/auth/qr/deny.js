...
const { denyQrLoginRequest } = require("../../_lib/qr-login");
...
module.exports = async function handler(req, res) {
  ...
  const endpoint = "/api/auth/qr/deny";
  try {
    ...
    await assertRateLimit("denyQrLoginRequest", user.uid);
    const data = await denyQrLoginRequest({ uid: user.uid, role: user.role, payload, context: { requestId: id, endpoint } });
