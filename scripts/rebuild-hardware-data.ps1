param(
  [string]$DocxPath = "D:\RTU\coderecall-project\Final Modules and Quiz\Computer Hardware\w_answer key.docx",
  [string]$QuizOutputPath = "C:\Users\robes\Downloads\coderecall-main\coderecall-main\data\quiz-data-hardware.js",
  [string]$PosttestOutputPath = "C:\Users\robes\Downloads\coderecall-main\coderecall-main\data\hardware-posttest-data.js"
)
  ...
  while ($i -lt $EndIndex) {
    ...
    if ($line -match "^(\d+)\.(\d+)\.?\s*(.*)$") {
      ...
      while ($i -lt $EndIndex) {
        ...
        if ($current -match "^(\d+)\.(\d+)\.?\s*" -or $current -like "COMPUTER HARDWARE (*") {
          break
        }
...
$easyIndex = [array]::IndexOf($lines, "COMPUTER HARDWARE (QUIZ - EASY)")
$mediumIndex = [array]::IndexOf($lines, "COMPUTER HARDWARE (QUIZ - MEDIUM)")
$hardIndex = [array]::IndexOf($lines, "COMPUTER HARDWARE (QUIZ - HARD)")
$postIndex = [array]::IndexOf($lines, "COMPUTER HARDWARE (POST-TEST)")

if ($easyIndex -lt 0 -or $mediumIndex -lt 0 -or $hardIndex -lt 0 -or $postIndex -lt 0) {
  throw "Could not find one or more hardware quiz section headers in the DOCX."
}
...
$hardwareQuizData = [pscustomobject]@{
...
foreach ($level in ($easyLevels.Keys | Sort-Object)) {
  $hardwareQuizData.hardware.easy.$level = @($easyLevels[$level] | Sort-Object sub | ForEach-Object {
    [pscustomobject]@{
      level = $level
      sub = $_.sub
      question = $_.question
...
foreach ($level in ($mediumLevels.Keys | Sort-Object)) {
  $hardwareQuizData.hardware.medium.$level = @($mediumLevels[$level] | Sort-Object sub | ForEach-Object {
    [pscustomobject]@{
      level = $level
      sub = $_.sub
      question = $_.question
...
foreach ($level in ($hardLevels.Keys | Sort-Object)) {
  $hardwareQuizData.hardware.hard.$level = @($hardLevels[$level] | Sort-Object sub | ForEach-Object {
    [pscustomobject]@{
      level = $level
      sub = $_.sub
      question = $_.question
...
$quizContent = "export const hardwareQuizData = " + (ConvertTo-JsLiteral $hardwareQuizData 0) + ";`n"
Set-Content -LiteralPath $QuizOutputPath -Value $quizContent -Encoding UTF8

$postContent = "export const hardwarePosttestQuestions = " + (ConvertTo-JsLiteral @($posttestItems) 0) + ";`n"
Set-Content -LiteralPath $PosttestOutputPath -Value $postContent -Encoding UTF8

Write-Output ("Rebuilt hardware quiz data: easy={0}, medium={1}, hard={2}" -f $easyLevels.Count, $mediumLevels.Count, $hardLevels.Count)
Write-Output ("Rebuilt hardware posttest questions: {0}" -f $posttestItems.Count)
