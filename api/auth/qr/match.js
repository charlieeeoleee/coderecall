...
const { matchQrLoginRequest } = require("../../_lib/qr-login");
...
module.exports = async function handler(req, res) {
  ...
  const endpoint = "/api/auth/qr/match";
  try {
    ...
    await assertRateLimit("matchQrLoginRequest", user.uid);
    const data = await matchQrLoginRequest({ uid: user.uid, role: user.role, payload, context: { requestId: id, endpoint } });
